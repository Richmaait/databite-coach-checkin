...
function CoachRosterCard({
  ...
  const [showCoachResetDialog, setShowCoachResetDialog] = useState(false);
  ...
  const handleResetClick = () => setShowCoachResetDialog(true);
          ...
          {rated > 0 && (
            <div className="relative">
              <button
                ...
                className="text-xs px-2.5 py-1 rounded-lg border bg-white/5 border-white/10 text-white/50 hover:border-red-400/30 hover:text-red-400 hover:bg-red-400/5 transition-all"
              >
                Reset all
              </button>
              {showCoachResetDialog && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setShowCoachResetDialog(false)} />
                  <div className="absolute z-50 top-full right-0 mt-2 bg-zinc-900 border border-white/15 rounded-xl p-4 shadow-2xl w-64">
                    <p className="text-sm font-semibold text-white/90 mb-2">Clear {coachName}&apos;s ratings?</p>
                    <p className="text-xs text-white/40 mb-3">A backup will be created so you can undo this.</p>
                    <div className="flex items-center gap-2 justify-end">
                      <button onClick={() => setShowCoachResetDialog(false)} className="px-3 py-1.5 rounded-lg text-xs text-white/50 hover:text-white/80 transition-colors">
                        Cancel
                      </button>
                      <button
                        onClick={() => { onReset(coachId); setShowCoachResetDialog(false); }}
...
export default function ClientProgress() {
  ...
  const [lastResetBackupId, setLastResetBackupId] = useState<number | null>(null);
  const [showResetConfirm, setShowResetConfirm] = useState(false);
  const [resetCoachId, setResetCoachId] = useState<number | null>(null);

  const resetAllMutation = trpc.performance.resetAllRatings.useMutation({
    onSuccess: (data) => {
      refetchAll();
      setLastResetBackupId(data.backupId);
      setShowResetConfirm(false);
      toast.success(`${data.backedUp} ratings cleared. You can undo this.`, { duration: 15000 });
    },
    onError: (err) => toast.error(err.message ?? "Failed to reset ratings"),
  });

  const undoResetMutation = trpc.performance.undoResetRatings.useMutation({
    onSuccess: (data) => {
      refetchAll();
      setLastResetBackupId(null);
      toast.success(`${data.restored} ratings restored!`);
    },
    onError: (err) => toast.error(err.message ?? "Failed to undo reset"),
  });
      ...
      <div className="flex flex-col gap-8 p-6 pt-20 max-w-5xl mx-auto">
          ...
          {isAdmin && (
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-xl border border-white/10 text-white/50 hover:border-red-400/30 hover:text-red-400 hover:bg-red-400/5 text-sm font-medium transition-colors"
              >
        ...
        {lastResetBackupId && (
          <div className="glass rounded-2xl p-4 flex items-center justify-between border border-amber-400/20">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm text-white/80">All ratings have been cleared.</span>
            </div>
            <button
              onClick={() => undoResetMutation.mutate({ backupId: lastResetBackupId })}
              disabled={undoResetMutation.isPending}
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-white/10 border border-white/20 text-white text-sm font-semibold hover:bg-white/15 transition-colors"
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 15 3 9m0 0 6-6M3 9h12a6 6 0 0 1 0 12h-3" />
              </svg>
              {undoResetMutation.isPending ? "Restoring..." : "Undo"}
            </button>
          </div>
        )}
        ...
        {showResetConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowResetConfirm(false)}>
            <div className="bg-zinc-900 border border-white/15 rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
              <h3 className="text-lg font-bold text-white/90 mb-2">Clear All Ratings?</h3>
              <p className="text-sm text-white/50 mb-4">
                This will remove all client ratings across all coaches. A backup will be created so you can undo this action immediately after.
              </p>
              <p className="text-xs text-white/30 mb-6">
                Tip: Generate a Post-Sweep Report first to save a permanent snapshot before clearing.
              </p>
              <div className="flex items-center gap-3 justify-end">
                ...
                <button
                  onClick={() => resetAllMutation.mutate({ coachId: resetCoachId ?? undefined })}
                  disabled={resetAllMutation.isPending}
                  className="px-4 py-2 rounded-xl bg-red-500 hover:bg-red-400 text-white text-sm font-semibold transition-colors shadow-lg shadow-red-500/20"
                >
                  {resetAllMutation.isPending ? "Clearing..." : "Yes, Clear All Ratings"}
                </button>
              </div>
            </div>
          </div>
