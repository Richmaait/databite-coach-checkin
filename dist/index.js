...
var users, coaches, checkinRecords, clientCheckIns, excusedClients, rosterClientStarts, rosterWeeklySnapshots, kudos, sweepReports, clientRatings, slackReminderLog, pausedClients, salesCheckins, fridayAudits, auditHistory, onboardingClients, rosterAssignments;
var init_schema = __esm({
  "drizzle/schema.ts"() {
    ...
    fridayAudits = mysqlTable("friday_audits", {
      id: int("id").autoincrement().primaryKey(),
      coachId: int("coachId").notNull(),
      coachName: varchar("coachName", { length: 128 }).notNull(),
      weekStart: varchar("weekStart", { length: 10 }).notNull(),
    ...
    auditHistory = mysqlTable("audit_history", {
      id: int("id").autoincrement().primaryKey(),
      coachId: int("coachId").notNull(),
      clientName: varchar("clientName", { length: 256 }).notNull(),
    ...
    rosterAssignments = mysqlTable("roster_assignments", {
      id: int("id").autoincrement().primaryKey(),
      coachId: int("coachId").notNull(),
      coachName: varchar("coachName", { length: 128 }).notNull(),
      clientName: varchar("clientName", { length: 256 }).notNull(),
      ...
      uqRosterAssignment: uniqueIndex("uq_roster_assignment").on(t2.coachId, t2.clientName, t2.dayOfWeek)
    }));
  }
});
...
async function fetchRosterForCoach(coachName) {
  ...
  const db2 = await getDb();
  if (db2) {
    const rows = await db2.select().from(rosterAssignments).where(and2(
      eq3(rosterAssignments.coachName, coachName),
      eq3(rosterAssignments.isActive, 1)
    ));
    if (rows.length > 0) {
      const days = { ...empty };
      for (const r of rows) {
...
async function fetchRosterForCoachFromSheet(coachName) {
  const empty = {
    monday: [],
    tuesday: [],
    wednesday: [],
...
async function fetchRawRosterForCoach(coachName) {
  ...
  const db2 = await getDb();
  if (db2) {
    const rows2 = await db2.select().from(rosterAssignments).where(and2(
      eq3(rosterAssignments.coachName, coachName),
      eq3(rosterAssignments.isActive, 1)
    ));
    if (rows2.length > 0) {
      const days2 = { ...empty };
      for (const r of rows2) days2[r.dayOfWeek].push(r.clientName);
      return days2;
    }
  }
  const rows = await fetchSheetRows();
...
var SHEET_ID, SHEET_TAB, DAYS2, _cachedRows, _cacheTime, CACHE_TTL_MS;
var init_rosterUtils = __esm({
  "server/rosterUtils.ts"() {
    "use strict";
    init_env();
    init_db();
    init_schema();
    SHEET_ID = "1puu4oLAmC5jV_GEmRrMxvXuTak_dl6pOJ6iWC44Nfl4";
    SHEET_TAB = "CLIENT ROSTER";
...
var slackFridayAudit_exports = {};
__export(slackFridayAudit_exports, {
  checkMissedAudits: () => checkMissedAudits,
...
async function sendFridayAudit() {
  ...
  for (const coach of allCoaches) {
    ...
    if (!claimed) continue;
    const existing = await db2.select().from(fridayAudits).where(and4(eq6(fridayAudits.coachId, coach.id), eq6(fridayAudits.weekStart, weekStart))).limit(1);
    ...
    const prevAudited = await db2.select().from(auditHistory).where(eq6(auditHistory.coachId, coach.id));
    ...
    if (totalEligible === 0) {
      await db2.delete(auditHistory).where(eq6(auditHistory.coachId, coach.id));
      for (const day of DAYS2) {
        const clients = roster[day] ?? [];
    ...
    for (const s of selected) {
      const existingHistory = await db2.select().from(auditHistory).where(and4(eq6(auditHistory.coachId, coach.id), eq6(auditHistory.clientName, s.name))).limit(1);
      if (existingHistory.length > 0) {
        await db2.update(auditHistory).set({ lastAuditedWeek: weekStart }).where(eq6(auditHistory.id, existingHistory[0].id));
      } else {
...
async function checkMissedAudits() {
  ...
  const weekStart = getMonday(getTodayMelbourne());
  const audits = await db2.select().from(fridayAudits).where(and4(eq6(fridayAudits.weekStart, weekStart), isNull2(fridayAudits.allSubmittedAt)));
  for (const audit of audits) {
    const [coach] = await db2.select().from(coaches).where(eq6(coaches.id, audit.coachId)).limit(1);
    if (!coach) continue;
    const timezone = coach.timezone ?? "Australia/Melbourne";
...
var init_slackFridayAudit = __esm({
  "server/slackFridayAudit.ts"() {
    "use strict";
    init_db();
    init_schema();
...
async function sendMondayDigest() {
  ...
  const snapshots = await db2.select().from(rosterWeeklySnapshots).where(eq7(rosterWeeklySnapshots.weekStart, lastWeekStart));
  ...
  for (const coach of allCoaches) {
    ...
    if (snap?.scheduled != null) {
      ...
      const liveExcuses = await db2.select().from(excusedClients).where(and5(
        eq7(excusedClients.coachId, coach.id),
        eq7(excusedClients.weekStart, lastWeekStart),
        eq7(excusedClients.status, "approved")
      ));
...
import { z } from "zod";
import { eq as eq8, and as and6, gte as gte3, lte as lte2, desc, sql, asc, isNull as isNull4, ne } from "drizzle-orm";
init_env();
init_schema();
...
// server/engagementStats.ts
init_schema();
init_rosterUtils();
import { and as and3, eq as eq4, isNull } from "drizzle-orm";
async function computeCoachWeekStats(db2, coachId, coachName, weekStart, opts = {}) {
  const completions = await db2.select().from(clientCheckIns).where(and3(eq4(clientCheckIns.coachId, coachId), eq4(clientCheckIns.weekStart, weekStart)));
  const excuses = await db2.select().from(excusedClients).where(and3(
    eq4(excusedClients.coachId, coachId),
    eq4(excusedClients.weekStart, weekStart),
    eq4(excusedClients.status, "approved")
  ));
  const completedByDay = { monday: 0, tuesday: 0, wednesday: 0, thursday: 0, friday: 0 };
  const excusedByDay = { monday: 0, tuesday: 0, wednesday: 0, thursday: 0, friday: 0 };
  const checkInClientsByDay = {
    monday: /* @__PURE__ */ new Set(),
    tuesday: /* @__PURE__ */ new Set(),
    wednesday: /* @__PURE__ */ new Set(),
    thursday: /* @__PURE__ */ new Set(),
    friday: /* @__PURE__ */ new Set()
  };
  let completed = 0;
  ...
  if (opts.preferSnapshot) {
    const snapRows = await db2.select().from(rosterWeeklySnapshots).where(and3(eq4(rosterWeeklySnapshots.coachId, coachId), eq4(rosterWeeklySnapshots.weekStart, weekStart))).limit(1);
  ...
  if (!rosterByDay) {
    ...
    const paused = await db2.select().from(pausedClients).where(and3(eq4(pausedClients.coachId, coachId), isNull(pausedClients.resumedAt)));
    const pausedSet = new Set(paused.map((p) => p.clientName));
    rosterByDay = { monday: 0, tuesday: 0, wednesday: 0, thursday: 0, friday: 0 };
    for (const d of DAYS2) {
      rosterByDay[d] = (roster[d] ?? []).filter((c) => !pausedSet.has(c)).length;
    }
  }
  const scheduledByDay = { monday: 0, tuesday: 0, wednesday: 0, thursday: 0, friday: 0 };
  for (const d of DAYS2) {
...
async function computeDisengagedClients(coachId, coachName, weekStart) {
  ...
  const completions = await db2.select().from(clientCheckIns).where(eq8(clientCheckIns.coachId, coachId));
  const approvedExcuses = await db2.select().from(excusedClients).where(and6(eq8(excusedClients.coachId, coachId), eq8(excusedClients.status, "approved")));
  const starts = await db2.select().from(rosterClientStarts).where(eq8(rosterClientStarts.coachId, coachId));
...
async function notifyManagerOfSubmission(coachId, submissionType, details) {
  ...
  try {
    ...
    const [coach] = await db2.select().from(coaches).where(eq8(coaches.id, coachId)).limit(1);
    if (!coach) return;
    const appUrl = ENV.appUrl || "https://coach.databite.com.au";
...
var checkinsRouter = t.router({
  ...
  submitMorning: protectedProcedure.input(
    ...
    const db2 = await requireDb();
    const existing = await db2.select().from(checkinRecords).where(and6(eq8(checkinRecords.coachId, input.coachId), eq8(checkinRecords.recordDate, input.recordDate))).limit(1);
    if (existing.length > 0) {
      ...
      }).where(eq8(checkinRecords.id, existing[0].id));
  ...
  todayByCoach: protectedProcedure.input(
    ...
    const db2 = await requireDb();
    const rows = await db2.select().from(checkinRecords).where(and6(eq8(checkinRecords.coachId, input.coachId), eq8(checkinRecords.recordDate, input.recordDate))).limit(1);
  ...
  aggregate: adminProcedure.input(
    ...
    const rows = await db2.select().from(checkinRecords).where(and6(gte3(checkinRecords.recordDate, startDate), lte2(checkinRecords.recordDate, endDate)));
  ...
  byDateRange: adminProcedure.input(
    ...
    }).from(checkinRecords).where(and6(gte3(checkinRecords.recordDate, input.startDate), lte2(checkinRecords.recordDate, input.endDate))).orderBy(desc(checkinRecords.recordDate));
    const flat = [];
    for (const r of rows) {
      if (r.morningSubmittedAt) {
        flat.push({
  ...
  lowMoodAlerts: adminProcedure.query(async () => {
    ...
    }).from(checkinRecords).leftJoin(coaches, eq8(checkinRecords.coachId, coaches.id)).where(and6(gte3(checkinRecords.recordDate, weekAgo), lte2(checkinRecords.moodScore, 2))).orderBy(desc(checkinRecords.recordDate));
    return rows;
  }),
  ...
  recentNotes: adminProcedure.query(async () => {
    ...
    const rows = await db2.select({
      ...
      disengagementSubmittedAt: checkinRecords.disengagementSubmittedAt,
      coachName: coaches.name
    }).from(checkinRecords).leftJoin(coaches, eq8(checkinRecords.coachId, coaches.id)).orderBy(desc(checkinRecords.recordDate)).limit(20);
    const notes = [];
    for (const r of rows) {
...
var clientCheckinsRouter = t.router({
  ...
  getRosterWeeklyStats: protectedProcedure.input(
    ...
    let coachList;
    if (input.coachId) {
      const [coach] = await db2.select().from(coaches).where(eq8(coaches.id, input.coachId)).limit(1);
      coachList = coach ? [{ id: coach.id, name: coach.name }] : [];
    } else {
      coachList = await db2.select({ id: coaches.id, name: coaches.name }).from(coaches).where(eq8(coaches.isActive, 1));
    }
    const results = [];
    const currentWeekMon = getMonday2(getTodayMelbourne2());
    for (const ws of weekStartList) {
      const isPastWeek = ws < currentWeekMon;
      for (const coach of coachList) {
        const stats = await computeCoachWeekStats(db2, coach.id, coach.name, ws, { preferSnapshot: isPastWeek });
        results.push({
          coachId: coach.id,
          coachName: coach.name,
          weekStart: ws,
    ...
    for (const coach of coachList) {
      ...
      for (const day of DAYS) {
        ...
        const completions = await db2.select().from(clientCheckIns).where(
          and6(
            eq8(clientCheckIns.coachId, coach.id),
            eq8(clientCheckIns.weekStart, input.weekStart),
            eq8(clientCheckIns.dayOfWeek, day)
          )
        );
  ...
  getActivityReport: adminProcedure.input(z.object({
    ...
    const records = await db2.select({
      ...
      disengagementSubmittedAt: checkinRecords.disengagementSubmittedAt,
      coachName: coaches.name
    }).from(checkinRecords).leftJoin(coaches, eq8(checkinRecords.coachId, coaches.id)).where(
      and6(
    ...
    for (const r of records) {
      ...
      const completions = dayKey ? await db2.select().from(clientCheckIns).where(
        and6(
          eq8(clientCheckIns.coachId, r.coachId),
          eq8(clientCheckIns.weekStart, weekStart),
          eq8(clientCheckIns.dayOfWeek, dayKey)
        )
      ) : [];
  ...
  getAllDisengagedClients: protectedProcedure.query(async () => {
    ...
    for (const coach of coachList) {
      const roster = await fetchRosterForCoach(coach.name);
      const completions = await db2.select().from(clientCheckIns).where(and6(eq8(clientCheckIns.coachId, coach.id)));
      const approvedExcuses = await db2.select().from(excusedClients).where(and6(eq8(excusedClients.coachId, coach.id), eq8(excusedClients.status, "approved")));
      ...
      const pausedSet = new Set(paused.map((p) => p.clientName));
      const starts = await db2.select().from(rosterClientStarts).where(eq8(rosterClientStarts.coachId, coach.id));
  ...
  getAllMissedStreaks: protectedProcedure.query(async () => {
    ...
    for (const coach of coachList) {
      ...
      const paused = await db2.select().from(pausedClients).where(and6(eq8(pausedClients.coachId, coach.id), isNull4(pausedClients.resumedAt)));
      const pausedSet = new Set(paused.map((p) => p.clientName));
      const completions = await db2.select().from(clientCheckIns).where(eq8(clientCheckIns.coachId, coach.id));
      const approvedExcuses = await db2.select().from(excusedClients).where(and6(eq8(excusedClients.coachId, coach.id), eq8(excusedClients.status, "approved")));
      const completionSet = /* @__PURE__ */ new Set();
      for (const c of completions) {
        if (c.completedAt == null) continue;
  ...
  getAllTodayMorning: adminProcedure.query(async () => {
    ...
    const rows = await db2.select({
      ...
      morningSubmittedAt: checkinRecords.morningSubmittedAt,
      coachName: coaches.name
    }).from(checkinRecords).leftJoin(coaches, eq8(checkinRecords.coachId, coaches.id)).where(and6(eq8(checkinRecords.recordDate, today), sql`${checkinRecords.morningSubmittedAt} IS NOT NULL`));
    return rows;
  }),
  ...
  getDailyActivityBreakdown: adminProcedure.input(
    ...
    const statsPerCoach = await Promise.all(coachList.map(async (coach) => ({
      coach,
      stats: await computeCoachWeekStats(db2, coach.id, coach.name, input.weekStart, { preferSnapshot: isPastWeek })
    })));
    const result = [];
  ...
  getExcuseCountsByCoach: adminProcedure.input(
    ...
    const db2 = await requireDb();
    const weekStart = input?.weekStart ?? getTodayMelbourne2().slice(0, 8) + "01";
    const rows = await db2.select({
      coachId: excusedClients.coachId,
      coachName: excusedClients.coachName,
      status: excusedClients.status
    }).from(excusedClients).where(eq8(excusedClients.weekStart, weekStart));
  ...
  getPendingExcuses: adminProcedure.query(async () => {
    const db2 = await requireDb();
    const rows = await db2.select().from(excusedClients).where(eq8(excusedClients.status, "pending")).orderBy(desc(excusedClients.submittedAt));
    return rows;
  }),
  ...
  getPerformanceReport: adminProcedure.input(
    ...
    for (const week of weeks) {
      ...
      for (const coach of coachList) {
        const stats = await computeCoachWeekStats(db2, coach.id, coach.name, week, { preferSnapshot: isPastWeek });
  ...
  getTodayPendingClients: protectedProcedure.input(
    ...
    const completions = await db2.select().from(clientCheckIns).where(
      and6(
        eq8(clientCheckIns.coachId, input.coachId),
        eq8(clientCheckIns.weekStart, weekStart),
        eq8(clientCheckIns.dayOfWeek, dayKey)
      )
    );
    ...
    const excuses = await db2.select().from(excusedClients).where(
      and6(
        eq8(excusedClients.coachId, input.coachId),
        eq8(excusedClients.weekStart, weekStart),
        eq8(excusedClients.dayOfWeek, dayKey),
        eq8(excusedClients.status, "approved")
      )
    );
  ...
  markComplete: protectedProcedure.input(
    ...
    const existing = await db2.select().from(clientCheckIns).where(
      and6(
        eq8(clientCheckIns.coachId, input.coachId),
        eq8(clientCheckIns.clientName, input.clientName),
        eq8(clientCheckIns.dayOfWeek, input.dayOfWeek),
        eq8(clientCheckIns.weekStart, input.weekStart)
      )
    ).limit(1);
    if (existing.length > 0) {
      ...
      }).where(eq8(clientCheckIns.id, existing[0].id));
  ...
  toggleClientSubmitted: protectedProcedure.input(
    ...
    if (!coachId) {
      const [myCoach] = await db2.select().from(coaches).where(eq8(coaches.userId, ctx.user.id)).limit(1);
    ...
    const existing = await db2.select().from(clientCheckIns).where(
      and6(
        eq8(clientCheckIns.coachId, coachId),
        eq8(clientCheckIns.clientName, input.clientName),
        eq8(clientCheckIns.dayOfWeek, input.dayOfWeek),
        eq8(clientCheckIns.weekStart, input.weekStart)
      )
  ...
  submitExcuse: protectedProcedure.input(
    ...
    const existing = await db2.select().from(excusedClients).where(
      and6(
        eq8(excusedClients.coachId, input.coachId),
        eq8(excusedClients.clientName, input.clientName),
        eq8(excusedClients.dayOfWeek, input.dayOfWeek),
        eq8(excusedClients.weekStart, input.weekStart)
      )
    ).limit(1);
    if (existing.length > 0) {
      throw new TRPCError({
...
Open the app to approve or reject.`;
  ...
  reviewExcuse: adminProcedure.input(
    ...
    }).where(eq8(excusedClients.id, input.id));
    return { success: true };
  }),
  ...
  getWeekStatusAll: protectedProcedure.input(z.object({ weekStart: z.string() })).query(async ({ input }) => {
    const db2 = await requireDb();
    const rows = await db2.select().from(clientCheckIns).where(eq8(clientCheckIns.weekStart, input.weekStart));
    return rows.map((r) => ({
      id: r.id,
      coachId: r.coachId,
      coachName: r.coachName,
  ...
  getActivePauses: protectedProcedure.input(z.object({ coachId: z.number() })).query(async ({ input }) => {
    const db2 = await requireDb();
    const rows = await db2.select().from(pausedClients).where(and6(eq8(pausedClients.coachId, input.coachId), isNull4(pausedClients.resumedAt)));
  ...
  pauseClient: protectedProcedure.input(z.object({ coachId: z.number(), clientName: z.string() })).mutation(async ({ input, ctx }) => {
    const db2 = await requireDb();
    const [existing] = await db2.select().from(pausedClients).where(and6(eq8(pausedClients.coachId, input.coachId), eq8(pausedClients.clientName, input.clientName), isNull4(pausedClients.resumedAt))).limit(1);
    if (existing) {
  ...
  resumeClient: protectedProcedure.input(z.object({ coachId: z.number(), clientName: z.string() })).mutation(async ({ input }) => {
    const db2 = await requireDb();
    await db2.update(pausedClients).set({ resumedAt: sql`NOW()` }).where(and6(eq8(pausedClients.coachId, input.coachId), eq8(pausedClients.clientName, input.clientName), isNull4(pausedClients.resumedAt)));
    return { ok: true };
  }),
  ...
  getExcusesForWeek: protectedProcedure.input(z.object({ weekStart: z.string(), coachId: z.number().optional() })).query(async ({ input }) => {
    const db2 = await requireDb();
    const conditions = [eq8(excusedClients.weekStart, input.weekStart)];
    if (input.coachId != null) {
      conditions.push(eq8(excusedClients.coachId, input.coachId));
    }
    const rows = await db2.select().from(excusedClients).where(and6(...conditions));
    return rows;
  }),
  ...
  importWeeklySnapshots: adminProcedure.input(z.array(z.object({
    ...
    for (const r of input) {
      ...
      const existing = await db2.select().from(rosterWeeklySnapshots).where(and6(eq8(rosterWeeklySnapshots.coachId, r.coachId), eq8(rosterWeeklySnapshots.weekStart, r.weekStart))).limit(1);
      if (existing.length > 0) {
  ...
  clientSelfCheckin: publicProcedure.input(
    ...
    const allCoaches = await db2.select().from(coaches).where(eq8(coaches.isActive, 1));
    let foundCoach = null;
    ...
    const existing = await db2.select().from(clientCheckIns).where(
      and6(
        eq8(clientCheckIns.coachId, foundCoach.id),
        eq8(clientCheckIns.clientName, foundClientName),
        eq8(clientCheckIns.dayOfWeek, dayOfWeek),
        eq8(clientCheckIns.weekStart, weekStart)
      )
    ).limit(1);
    if (existing.length > 0) {
      ...
      await db2.update(clientCheckIns).set({ clientSubmitted: 1, clientSubmittedAt: /* @__PURE__ */ new Date() }).where(eq8(clientCheckIns.id, existing[0].id));
...
var coachesRouter = t.router({
  ...
  list: protectedProcedure.query(async () => {
    const db2 = await requireDb();
    return db2.select().from(coaches).where(eq8(coaches.isActive, 1)).orderBy(asc(coaches.name));
  }),
  /** Coach profile linked to current user. */
  myCoach: protectedProcedure.query(async ({ ctx }) => {
    const db2 = await requireDb();
    const [coach] = await db2.select().from(coaches).where(eq8(coaches.userId, ctx.user.id)).limit(1);
    return coach ?? null;
  }),
  /** Coaches with no userId linked. */
  unclaimed: protectedProcedure.query(async () => {
    const db2 = await requireDb();
    return db2.select().from(coaches).where(and6(eq8(coaches.isActive, 1), isNull4(coaches.userId)));
  }),
  ...
  streaks: protectedProcedure.query(async () => {
    ...
    const results = [];
    for (const coach of coachList) {
      const records = await db2.select({ recordDate: checkinRecords.recordDate, morningSubmittedAt: checkinRecords.morningSubmittedAt }).from(checkinRecords).where(eq8(checkinRecords.coachId, coach.id)).orderBy(desc(checkinRecords.recordDate));
  ...
  update: adminProcedure.input(
    ...
    if (Object.keys(setObj).length > 0) {
      await db2.update(coaches).set(setObj).where(eq8(coaches.id, id));
    }
    return { success: true };
  }),
  ...
  claimProfile: protectedProcedure.input(
    ...
    const db2 = await requireDb();
    const [existing] = await db2.select().from(coaches).where(eq8(coaches.id, input.coachId)).limit(1);
    ...
    await db2.update(coaches).set({ userId: ctx.user.id, email: ctx.user.email }).where(eq8(coaches.id, input.coachId));
    return { success: true };
  }),
  ...
  linkUser: adminProcedure.input(
    ...
    const db2 = await requireDb();
    await db2.update(coaches).set({ userId: input.userId }).where(eq8(coaches.id, input.coachId));
    return { success: true };
  }),
...
var performanceRouter = t.router({
  ...
  myRatings: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [myCoach] = await db2.select().from(coaches).where(eq8(coaches.userId, ctx.user.id)).limit(1);
    if (!myCoach) return [];
    return db2.select().from(clientRatings).where(eq8(clientRatings.coachId, myCoach.id)).orderBy(asc(clientRatings.clientName));
  }),
  ...
  getWeeklySummary: adminProcedure.input(
    ...
    const db2 = await requireDb();
    const coachList = await db2.select({ id: coaches.id, name: coaches.name, workdays: coaches.workdays }).from(coaches).where(eq8(coaches.isActive, 1));
    ...
    const snapshots = isPastWeek ? await db2.select().from(rosterWeeklySnapshots).where(eq8(rosterWeeklySnapshots.weekStart, input.weekStart)) : [];
    const snapMap = new Map(snapshots.map((s) => [s.coachId, s.snapshotJson]));
    for (const coach of coachList) {
      const snap = snapMap.get(coach.id);
      let scheduled;
      ...
      if (isPastWeek && snap?.scheduled != null) {
        scheduled = snap.scheduled;
        completed = snap.completed ?? 0;
        const liveExcuses = await db2.select().from(excusedClients).where(and6(
          eq8(excusedClients.coachId, coach.id),
          eq8(excusedClients.weekStart, input.weekStart),
          eq8(excusedClients.status, "approved")
        ));
        excusedCount = liveExcuses.length;
        ...
        const completions = await db2.select().from(clientCheckIns).where(and6(eq8(clientCheckIns.coachId, coach.id), eq8(clientCheckIns.weekStart, input.weekStart)));
      ...
      const records = await db2.select().from(checkinRecords).where(
        and6(
          eq8(checkinRecords.coachId, coach.id),
    ...
    const prevSnapshots = await db2.select().from(rosterWeeklySnapshots).where(eq8(rosterWeeklySnapshots.weekStart, prevWeekStart));
    ...
    for (const coach of coachList) {
      ...
      if (prevSnap?.scheduled != null) {
        ...
        let scheduled = 0;
        for (const day of DAYS) scheduled += (roster[day] ?? []).length;
        const completions = await db2.select().from(clientCheckIns).where(and6(eq8(clientCheckIns.coachId, coach.id), eq8(clientCheckIns.weekStart, prevWeekStart)));
        const completed = completions.filter((c) => c.completedAt != null).length;
  ...
  setRating: protectedProcedure.input(
    ...
    const existing = await db2.select().from(clientRatings).where(
      and6(eq8(clientRatings.coachId, input.coachId), eq8(clientRatings.clientName, input.clientName))
    ).limit(1);
    if (existing.length > 0) {
      await db2.update(clientRatings).set({ rating: input.rating, notes: input.notes ?? existing[0].notes }).where(eq8(clientRatings.id, existing[0].id));
      return { id: existing[0].id };
    }
  ...
  resetAllRatings: adminProcedure.input(z.object({ coachId: z.number().optional() }).optional()).mutation(async ({ input }) => {
    const db2 = await requireDb();
    const ratingsToBackup = input?.coachId ? await db2.select().from(clientRatings).where(eq8(clientRatings.coachId, input.coachId)) : await db2.select().from(clientRatings);
    ...
    if (input?.coachId) {
      await db2.delete(clientRatings).where(eq8(clientRatings.coachId, input.coachId));
    } else {
  ...
  undoResetRatings: adminProcedure.input(z.object({ backupId: z.number() })).mutation(async ({ input }) => {
    const db2 = await requireDb();
    const [backup] = await db2.select().from(sweepReports).where(eq8(sweepReports.id, input.backupId)).limit(1);
    ...
    for (const r of snapshot.ratings) {
      const existing = await db2.select().from(clientRatings).where(and6(eq8(clientRatings.coachId, r.coachId), eq8(clientRatings.clientName, r.clientName))).limit(1);
      if (existing.length > 0) {
        await db2.update(clientRatings).set({ rating: r.rating, notes: r.notes }).where(eq8(clientRatings.id, existing[0].id));
      } else {
    ...
    await db2.delete(sweepReports).where(eq8(sweepReports.id, input.backupId));
...
var sweepReportRouter = t.router({
  ...
  create: adminProcedure.input(
    ...
    const db2 = await requireDb();
    const coachList = input.coachId ? await db2.select().from(coaches).where(eq8(coaches.id, input.coachId)) : await db2.select().from(coaches).where(eq8(coaches.isActive, 1));
    ...
    for (const coach of coachList) {
      ...
      const ratings = await db2.select().from(clientRatings).where(eq8(clientRatings.coachId, coach.id));
  ...
  save: adminProcedure.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db2 = await requireDb();
    await db2.update(sweepReports).set({ isSaved: 1 }).where(eq8(sweepReports.id, input.id));
    return { success: true };
  }),
  /** Get report by ID (public for sharing). */
  getById: publicProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    const db2 = await requireDb();
    const [report] = await db2.select().from(sweepReports).where(eq8(sweepReports.id, input.id)).limit(1);
  ...
  listSaved: adminProcedure.query(async () => {
    const db2 = await requireDb();
    return db2.select().from(sweepReports).where(eq8(sweepReports.isSaved, 1)).orderBy(desc(sweepReports.createdAt));
  }),
  /** Compare two reports. */
  compare: adminProcedure.input(
    ...
    const db2 = await requireDb();
    const [reportA] = await db2.select().from(sweepReports).where(eq8(sweepReports.id, input.idA)).limit(1);
    const [reportB] = await db2.select().from(sweepReports).where(eq8(sweepReports.id, input.idB)).limit(1);
    if (!reportA || !reportB) {
...
var usersRouter = t.router({
  ...
  updateRole: adminProcedure.input(
    ...
    const db2 = await requireDb();
    await db2.update(users).set({ role: input.role }).where(eq8(users.id, input.id));
    return { success: true };
  })
});
...
${input.message}`;
  ...
  history: adminProcedure.query(async () => {
    ...
    return db2.select({
      ...
      coachName: coaches.name
    }).from(kudos).leftJoin(coaches, eq8(kudos.coachId, coaches.id)).orderBy(desc(kudos.createdAt)).limit(50);
  })
});
var salesRouter = t.router({
  ...
  submitMorning: protectedProcedure.input(z.object({
    ...
    const db2 = await requireDb();
    const existing = await db2.select().from(salesCheckins).where(and6(eq8(salesCheckins.userId, ctx.user.id), eq8(salesCheckins.recordDate, input.recordDate))).limit(1);
    if (existing.length > 0) {
      await db2.update(salesCheckins).set({
        ...
        morningNotes: input.morningNotes ?? existing[0].morningNotes,
        morningSubmittedAt: /* @__PURE__ */ new Date()
      }).where(eq8(salesCheckins.id, existing[0].id));
    } else {
...
\u{1F449} <${appUrl}/sales|View Sales>`;
  ...
  getToday: protectedProcedure.input(z.object({ recordDate: z.string() })).query(async ({ input, ctx }) => {
    const db2 = await requireDb();
    const [record] = await db2.select().from(salesCheckins).where(and6(eq8(salesCheckins.userId, ctx.user.id), eq8(salesCheckins.recordDate, input.recordDate))).limit(1);
    return record ?? null;
  }),
  ...
  getAll: adminProcedure.input(z.object({ startDate: z.string().optional(), endDate: z.string().optional() }).optional()).query(async ({ input }) => {
    ...
    return db2.select().from(salesCheckins).where(conditions.length > 0 ? and6(...conditions) : void 0).orderBy(desc(salesCheckins.recordDate));
  })
});
var auditsRouter = t.router({
  ...
  getMyAudit: protectedProcedure.input(z.object({ weekStart: z.string() })).query(async ({ input, ctx }) => {
    ...
    if (coachId) {
      const [audit] = await db2.select().from(fridayAudits).where(and6(eq8(fridayAudits.coachId, coachId), eq8(fridayAudits.weekStart, input.weekStart))).limit(1);
      return audit ?? null;
    }
    return null;
  }),
  ...
  getAllForWeek: adminProcedure.input(z.object({ weekStart: z.string() })).query(async ({ input }) => {
    const db2 = await requireDb();
    return db2.select().from(fridayAudits).where(eq8(fridayAudits.weekStart, input.weekStart));
  }),
  /** Admin: mark audit as reviewed. */
  markReviewed: adminProcedure.input(z.object({ auditId: z.number() })).mutation(async ({ input, ctx }) => {
    const db2 = await requireDb();
    await db2.update(fridayAudits).set({ reviewedAt: /* @__PURE__ */ new Date(), reviewedByUserId: ctx.user.id }).where(eq8(fridayAudits.id, input.auditId));
    return { success: true };
  }),
  ...
  addClientToAudit: adminProcedure.input(z.object({ auditId: z.number(), clientName: z.string(), day: z.string() })).mutation(async ({ input }) => {
    const db2 = await requireDb();
    const [audit] = await db2.select().from(fridayAudits).where(eq8(fridayAudits.id, input.auditId)).limit(1);
    if (!audit) throw new TRPCError({ code: "NOT_FOUND" });
    const clients = audit.selectedClients;
    clients.push({ name: input.clientName, day: input.day, submitted: false });
    await db2.update(fridayAudits).set({ selectedClients: clients, allSubmittedAt: null }).where(eq8(fridayAudits.id, input.auditId));
    return { success: true };
  }),
  ...
  submitClient: protectedProcedure.input(z.object({
    ...
    }).where(eq8(fridayAudits.id, input.auditId));
    if (allDone) {
      const managerSlackId = ENV.managerSlackId;
      if (managerSlackId && ENV.slackBotToken) {
        const ratingEmojis = { green: "\u{1F7E2} On Track", yellow: "\u{1F7E1} Neutral", red: "\u{1F534} Off Track" };
        const summary = clients.map(
          (c) => `\u2022 *${c.name}* (${c.day}) \u2014 ${ratingEmojis[c.rating ?? ""] ?? "No rating"}${c.loomLink ? ` \u2014 <${c.loomLink}|Loom/Fireflies>` : ""}${c.notes ? `
  _${c.notes}_` : ""}`
        ).join("\n");
...
var onboardingRouter = t.router({
  ...
  update: adminProcedure.input(z.object({
    ...
    await db2.update(onboardingClients).set(update).where(eq8(onboardingClients.id, id));
    return { ok: true };
  }),
...
var rosterRouter = t.router({
  list: adminProcedure.input(z.object({ coachId: z.number().optional() }).optional()).query(async ({ input }) => {
    const db2 = await requireDb();
    const conditions = [eq8(rosterAssignments.isActive, 1)];
    if (input?.coachId) conditions.push(eq8(rosterAssignments.coachId, input.coachId));
    return db2.select().from(rosterAssignments).where(and6(...conditions)).orderBy(rosterAssignments.coachName, rosterAssignments.dayOfWeek, rosterAssignments.clientName);
  }),
  ...
  unassign: adminProcedure.input(z.object({ id: z.number() })).mutation(async ({ input }) => {
    const db2 = await requireDb();
    await db2.update(rosterAssignments).set({ isActive: 0 }).where(eq8(rosterAssignments.id, input.id));
    return { ok: true };
  }),
  move: adminProcedure.input(z.object({
    ...
    const db2 = await requireDb();
    await db2.update(rosterAssignments).set({ dayOfWeek: input.dayOfWeek }).where(eq8(rosterAssignments.id, input.id));
    return { ok: true };
  })
});
...
async function markClientSubmitted(params) {
  ...
  const existing = await db2.select().from(clientCheckIns).where(
    and7(
      eq9(clientCheckIns.coachId, params.coachId),
      eq9(clientCheckIns.clientName, params.clientName),
      eq9(clientCheckIns.dayOfWeek, params.dayOfWeek),
      eq9(clientCheckIns.weekStart, params.weekStart)
    )
  ).limit(1);
  if (existing.length > 0) {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const coachRows = await db2.select().from(coaches).where(eq9(coaches.name, coachName)).limit(1);
        if (!coachRows.length) {
...
async function snapshotCurrentWeek() {
  ...
  const coachList = await db2.select({ id: coaches.id, name: coaches.name }).from(coaches).where(eq10(coaches.isActive, 1));
  for (const coach of coachList) {
    const stats = await computeCoachWeekStats(db2, coach.id, coach.name, weekStart);
    const snap = {
      scheduled: stats.scheduled,
      completed: stats.completed,
      excused: stats.excused,
    ...
    const existing = await db2.select().from(rosterWeeklySnapshots).where(and8(eq10(rosterWeeklySnapshots.coachId, coach.id), eq10(rosterWeeklySnapshots.weekStart, weekStart))).limit(1);
    if (existing.length > 0) {
      await db2.update(rosterWeeklySnapshots).set({ snapshotJson: snap }).where(eq10(rosterWeeklySnapshots.id, existing[0].id));
    } else {
