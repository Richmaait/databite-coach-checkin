...
export const clientRatings = mysqlTable("client_ratings", {
  ...
  coachId: int("coachId").notNull(),
  clientName: varchar("clientName", { length: 256 }).notNull(),
  rating: mysqlEnum("rating", ["green", "yellow", "red", "blue"]).notNull(),
  notes: text("notes"),
...
export const fridayAudits = mysqlTable("friday_audits", {
  ...
  coachName: varchar("coachName", { length: 128 }).notNull(),
  weekStart: varchar("weekStart", { length: 10 }).notNull(),
  selectedClients: json("selectedClients").$type<Array<{ name: string; day: string; loomLink?: string; notes?: string; rating?: "green" | "yellow" | "red" | "blue"; submitted?: boolean }>>().notNull(),
